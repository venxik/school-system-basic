...
class Logger {
  ...
  public error(message: string, error?: any): void {
    ...
    appLogger.error(errorMessage, { filename: this.filename });
  }

  public warn(message: string, error?: any): void {
    const warnMessage = error ? `${message} - ${error.message || error}` : message;
    appLogger.warn(warnMessage, { filename: this.filename });
  }

  public info(message: string, error?: any): void {
    const infoMessage = error ? `${message} - ${error.message || error}` : message;
    appLogger.info(infoMessage, { filename: this.filename });
  }
